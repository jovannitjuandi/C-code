...
void print (struct pod *first_pod);
int greater (struct date *large, struct date *small);
...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
   ...
   while (first_pod != NULL) {
      if (greater (first_pod -> when, start_day) == 1 && greater (finish_day, first_pod -> when) == 1) {
         print (first_pod);
      }
...
void print (struct pod *first_pod) {
   printf ("%02d/%02d/%02d %2d %s\n" , first_pod -> when -> day, first_pod -> when -> month, first_pod -> when -> year
            , first_pod -> how_many, first_pod -> species);
}

int greater (struct date *large, struct date *small) {
   int great = 0;
   if (large -> year < small -> year) {
      great = 0;
   } else {
      if ((large -> year == small -> year) && (large -> month < small -> month)) {
         great = 0;
      } else {
         if ((large -> day < small -> day) && (large -> month == small -> month)) {
            great = 0;
         } else {
            great = 1;
         }
