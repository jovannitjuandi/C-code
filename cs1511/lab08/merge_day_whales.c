...
int combine (struct pod *one, struct pod *two);
...
void merge_day_whales(struct pod *first_pod) {
    ...
    while (head != NULL) {
        struct pod *curr = head;
        while (curr -> next != NULL) {
            if (combine (head, curr -> next) == 1) {
               struct pod *del = curr -> next;
               head -> how_many = head -> how_many + del -> how_many;
               curr -> next = del -> next;
               free(del->when);
               free(del->species);
               free(del);
...
int combine (struct pod *one, struct pod *two) {
    if (strcmp (one->species, two->species) == 0) {
        if (one->when->day == two->when->day &&
            one->when->month == two->when->month &&
            one->when->year == two->when->year) {
            return 1;
        }
    }
    return 0;
