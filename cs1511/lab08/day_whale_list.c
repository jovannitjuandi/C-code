...
int compare_date (struct pod *first_pod, struct date *day);
...
void day_whales(struct pod *first_pod, struct date *day) {
    ...
    while (first_pod != NULL) {
        if (compare_date (first_pod, day) == 1) {
...
int compare_date (struct pod *first_pod, struct date *day) {
    ...
    int date = first_pod -> when -> day;
    int month = first_pod -> when -> month;
    int year = first_pod -> when -> year;

    if (date == day -> day && month == day -> month && year == day -> year) {
        return 1;
    } else {
        return 0;
    }
