...
int check_between (int start, int finish, int date);
...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
int start = evaluate_date (start_day);
int finish = evaluate_date (finish_day);
int date = 0;

   while (first_pod != NULL) {
    // PUT YOUR CODE HERE
      date = evaluate_date (first_pod -> when);
      if (check_between (start,finish, date) == 1) {
