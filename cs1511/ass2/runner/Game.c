...
int isValidMove(Game game, playerMove move) {
   if (move.action == DRAW_CARD) { // DRAW_CARD
      ...
      if (cardValue(topDiscard(game)) == DRAW_TWO) {
      ...
      } else if (cardValue(topDiscard(game)) == D) {
         ...
         if (cardValue(topDiscard(game)) == cardValue(move.card) || cardValue(move.card) == ZERO || cardColor(topDiscard(game)) == cardColor(move.card)
...
void playMove(Game game, playerMove move) {
   if (move.action == PLAY_CARD) {
      ...
      if (cardValue (topDiscard(game)) == A && getTopDiscardTurnNumber(game) == currentTurn(game)) {
         current = current + 2;
      } else {
         current = current + 1;
      }
