...
//
// Version 1.0.3: Remove MSG_MORE to improve compatibility with OSX.
// Version 1.0.2: Add extra clarification about server URL.
...
int main(int argc, char *argv[]) {
    ...
    printf("[SERVER] Access this server at http://%s:%d/\n",
            hostname, port);
    printf("[SERVER] Or if you're running this on a CSE server: "
            "http://%s.cse.unsw.edu.au:%d/\n",
            hostname, port);
    ...
    while (numberServed < NUMBER_OF_PAGES_TO_SERVE) {
        ...
        // Echo the entire request to the console for debugging.
...
int makeServerSocket(int portNumber) {
    ...
    // Bind the socket to the listening port (passed into this function)
    ...
    // Otherwise we might have to wait a little while before rerunning
    // the server once it has stopped.
...
void sendImage(int socket, struct pixel pixels[TILE_SIZE][TILE_SIZE]) {
    ...
    // Send the header
    send(socket, headerBuffer, PIXEL_START, 0);
