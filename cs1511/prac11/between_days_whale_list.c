...
int afterDate (struct date *first, struct date *second);
...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
   while (first_pod != NULL) {
      if (afterDate(finish_day, first_pod -> when) == 1 && afterDate(first_pod -> when, start_day) == 1) {
         printf ("%02d/%02d/%02d %2d %s\n", first_pod -> when -> day, first_pod -> when -> month, 
         first_pod -> when -> year, first_pod -> how_many, first_pod -> species);
      }
