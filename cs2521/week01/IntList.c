...
void IntListInsertInOrder (IntList L, int v)
{
	...
	**/
	...
	// second case: smallest value
	else if (v < L->first->data) {
	    n->next = L->first;
	    L->first = n;
	}
	
	// third case: largest value
	else if (v >= L->last->data) {
	    L->last->next = n;
	    L->last = n;
	}
	
	// fourth case: second smallest value
	else if (v < L->first->next->data) {
	    n->next = L->first->next;
	    L->first->next = n;
	}
	
	// general case: anywhere else
	else {
	    // find position
	    // go through the list (p is previous and c is current)
	    struct IntListNode *p = L->first;
	    struct IntListNode *c = L->first->next;
	    while (c->data < v) {
	        p = p->next;
	        c = c->next;
	    }
