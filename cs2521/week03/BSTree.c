...
// TODO
{
    ...
    Queue printOrder = newQueue();
    QueueJoin(printOrder, t);
    
    while (!QueueIsEmpty(printOrder)) {
        BSTree current = QueueLeave(printOrder);
        showBSTreeNode(current);
        
        if (current->left != NULL) {
            QueueJoin(printOrder, current->left);
        } 
        
        if (current->right != NULL) {
            QueueJoin(printOrder, current->right);
        }
    }
}
